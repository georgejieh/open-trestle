...
func readLocalGitRevision(ctx context.Context, reader localGitObjectReader, revision evidence.RevisionIdentity, limits localGitRevisionLimits) (LocalGitRevisionResult, error) {
	...
	walker := localGitRevisionWalker{
		...
		objectBytes:       int64(len(rootTreeContent)),
	}
	if walker.objectBytes > limits.maxObjectBytes {
	...
	for _, entry := range graph.Entries() {
		...
		totalContentBytes, err = checkedLocalGitRevisionAdd(totalContentBytes, int64(len(content)), limits.maxResultBytes)
		if err != nil {
			return LocalGitRevisionResult{}, LocalGitRevisionResourceLimit
		}
...
func (w *localGitRevisionWalker) walkTree(digest string, prefix []byte, depth int) error {
	...
	if !exists {
		...
		w.objectBytes, err = checkedLocalGitRevisionAdd(w.objectBytes, int64(len(content)), w.limits.maxObjectBytes)
		if err != nil {
			return LocalGitRevisionResourceLimit
		}
...
func validLocalGitRevisionLimits(limits localGitRevisionLimits) bool {
	return limits.maxDepth >= 0 && limits.maxEntries >= 0 && limits.maxUniqueTrees >= 1 && limits.maxUniqueBlobs >= 0 && limits.maxPathBytes >= 0 && limits.maxTotalPathBytes >= 0 && limits.maxObjectBytes >= 0 && limits.maxResultBytes >= 0
}
